...
describe('calculateAverageHeightDebounced', () => {
    ...
    it('should return null when not in browser', () => {
        ...
        const result = calculateAverageHeightDebounced(false, null, () => ({ start: 0, end: 10 }), [], {}, -1, 40, onUpdate);
    ...
    it('should call onUpdate with new height when conditions are met', async () => {
        ...
        const onUpdate = vi.fn();
        const timeoutId = calculateAverageHeightDebounced(false, null, () => ({ start: 0, end: 10 }), mockElements, {}, -1, 40, onUpdate);
    ...
    it('should not call onUpdate when height difference is small', () => {
        ...
        const onUpdate = vi.fn();
        calculateAverageHeightDebounced(false, null, () => ({ start: 0, end: 10 }), mockElements, {}, -1, 40, onUpdate);
        vi.advanceTimersByTime(200);
        expect(onUpdate).not.toHaveBeenCalled();
    });
    it('should respect the debounce time', () => {
        ...
        calculateAverageHeightDebounced(false, null, () => ({ start: 0, end: 10 }), mockElements, {}, -1, 40, onUpdate, customDebounceTime);
    ...
    it('should return null when isCalculatingHeight is true', () => {
        const onUpdate = vi.fn();
        const result = calculateAverageHeightDebounced(true, // isCalculatingHeight = true
        null, () => ({ start: 0, end: 10 }), [], {}, -1, 40, onUpdate);
        expect(result).toBeNull();
        expect(onUpdate).not.toHaveBeenCalled();
    });
    it('should return null when heightUpdateTimeout exists', () => {
        ...
        const result = calculateAverageHeightDebounced(false, existingTimeout, () => ({ start: 0, end: 10 }), [], {}, -1, 40, onUpdate);
        expect(result).toBeNull();
        expect(onUpdate).not.toHaveBeenCalled();
    });
    it('should return null when currentIndex equals lastMeasuredIndex', () => {
        const onUpdate = vi.fn();
        const result = calculateAverageHeightDebounced(false, null, () => ({ start: 5, end: 10 }), [], {}, 5, // matches start index
        40, onUpdate);
        expect(result).toBeNull();
        expect(onUpdate).not.toHaveBeenCalled();
    });
